...
-----------------------------------------------------------
-- NOTE:
-- (0) Assumeption: all global definitions must have no indentation.
-- (1) newline is not a whitespace
-- (2) "\n" is not a newline unless the following character is alphabet
-- As a result, we need to manually parse newlines.

realNewLine :: Stream s m Char => ParsecT s u m ()
realNewLine = try $ char '\n' >> lookAhead (satisfy isAlpha) >> return ()

fakeNewLine :: Stream s m Char => ParsecT s u m ()
fakeNewLine = try $ char '\n' >> notFollowedBy (satisfy isAlpha) >> return ()

isSpaceWithoutNL :: Char -> Bool
isSpaceWithoutNL c = isSpace c && c /= '\n'

realSimpleSpace :: Stream s m Char => ParsecT s u m ()
realSimpleSpace =
  skipMany1 ((satisfy isSpaceWithoutNL >> return ()) <|> fakeNewLine)
